...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static int height_impl(const Node *node) {
    ...
    int left_h = height_impl(node->left);
    int right_h = height_impl(node->right);
  ...
  static bool check_sorting_helper(const Node *node,
                                  const T *min_val,
                                  const T *max_val,
                                  Compare less) {
    if (node == nullptr) {
    ...
    if (min_val && !less(*min_val, node->datum)) {
      return false; // node must be greater than min_val
    }
    if (max_val && !less(node->datum, *max_val)) {
      return false; // node must be less than max_val
    }

    return check_sorting_helper(node->left, min_val, &node->datum, less) &&
          check_sorting_helper(node->right, &node->datum, max_val, less);
  }


  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    return check_sorting_helper(node, nullptr, nullptr, less);
  }
